...
// Private functions
void send_packet_no_fwd(uint32_t controller_id, unsigned char *data, unsigned int len);

// Function pointers
static void(*send_func)(uint32_t controller_id, unsigned char *data, unsigned int len) = 0;
...
static void(*rx_value_selective_func)(uint32_t mode, mc_values *values) = 0;
...
void bldc_interface_send_packet(uint32_t controller_id, unsigned char *data, unsigned int len) {
	if (send_func) {
		send_func(controller_id, data, len);
	}
}
...
void bldc_interface_set_current(uint32_t controller_id, float current) {
	if (motor_control_set_func) {
		motor_control_set_func(controller_id, MOTOR_CONTROL_CURRENT, current);
		return;
	}
	int32_t send_index = 0;
...
void bldc_interface_set_current_brake(uint32_t controller_id, float current) {
	if (motor_control_set_func) {
		motor_control_set_func(controller_id, MOTOR_CONTROL_CURRENT_BRAKE, current);
		return;
	}
	int32_t send_index = 0;
...
void bldc_interface_set_rpm(uint32_t controller_id, int rpm) {
	if (motor_control_set_func) {
		motor_control_set_func(controller_id, MOTOR_CONTROL_RPM, rpm);
		return;
	}
	int32_t send_index = 0;
...
void bldc_interface_set_pos(uint32_t controller_id, float pos) {
	if (motor_control_set_func) {
		motor_control_set_func(controller_id, MOTOR_CONTROL_POS, pos);
		return;
	}
	int32_t send_index = 0;
...
void bldc_interface_set_mcconf(uint32_t controller_id, const mc_configuration *mcconf) {
	...
	buffer_append_uint32(send_buffer, (uint32_t)MCCONF_SIGNATURE, &ind);
...
void bldc_interface_get_values(uint32_t controller_id) {
	if (values_requested_func) {
		values_requested_func(controller_id);
		return;
	}
	int32_t send_index = 0;
...
void bldc_interface_get_values_selective(uint32_t controller_id, COMM_GET_VALUE_SELECTIVE_t value_bit_pos){
	int32_t send_index = 0;
	uint32_t mask = 0;
	mask = (((uint32_t)1 << value_bit_pos) & 0x000FFFFF);
...
void  send_packet_no_fwd(uint32_t controller_id, unsigned char *data, unsigned int len) {
	if (!forward_func) {
		bldc_interface_send_packet(controller_id, data, len);
	}
}
...
//	switch (mode) {
//	case 0:
//		sprintf(zzz, "temp_fet_filtered : %.2f degC\r\n", val->temp_mos);
//		break;
//	case 1:
//		sprintf(zzz, "temp_motor_filtered : %.2f degC\r\n", val->temp_motor);
//		break;
//	case 2:
//		sprintf(zzz, "avg_motor_current : %.2f A\r\n", val->current_motor);
//		break;
//	case 3:
//		sprintf(zzz, "avg_input_current : %.2f A\r\n", val->current_in);
//		break;
//	case 4:
//		sprintf(zzz, "avg_id : %.2f A\r\n", val->id);
//		break;
//	case 5:
//		sprintf(zzz, "avg_iq : %.2f A\r\n", val->iq);
//		break;
//	case 6:
//		sprintf(zzz, "duty_cycle_now : %.2f %\r\n", val->duty_now);
//		break;
//	case 7:
//		sprintf(zzz, "rpm : %.2f\r\n", val->rpm/vesc.pole_pairs);
//		break;
//	case 8:
//		sprintf(zzz, "input_voltage : %.2f V\r\n", val->v_in);
//		break;
//	case 9:
//		sprintf(zzz, "amp_hours : %.4f Ah\r\n", val->amp_hours);
//		break;
//	case 10:
//		sprintf(zzz, "amp_hours_charged : %.4f degC\r\n", val->amp_hours_charged);
//		break;
//	case 11:
//		sprintf(zzz, "watt_hours : %.4f Wh\r\n", val->watt_hours);
//		break;
//	case 12:
//		sprintf(zzz, "watt_hours_charged : %.4f Wh\r\n", val->watt_hours_charged);
//		break;
//	case 13:
//		sprintf(zzz, "tachometer_value : %i degC\r\n", val->tachometer);
//		break;
//	case 14:
//		sprintf(zzz, "tachometer_abs_value : %i degC\r\n", val->tachometer_abs);
//		break;
//	case 15:
//		sprintf(zzz, "fault : %s\r\n", bldc_interface_fault_to_string(val->fault_code));
//		break;
//	case 16:
//		sprintf(zzz, "pid_pos_now : %.2f deg\r\n", val->pid_pos);
//		break;
//	case 17:
//		sprintf(zzz, "vesc_id : %d\r\n", val->vesc_id);
//		break;
//	case 18:
//		sprintf(zzz, "temp_mos1 : %.2f degC\ttemp_mos2 : %.2f degC\ttemp_mos3 : %.2f degC\r\n", val->temp_mos1, val->temp_mos2, val->temp_mos3);
//		break;
//	case 19:
//		sprintf(zzz, "avg_vd : %.2f V\r\n", val->vd);
//		break;
//	case 20:
//		sprintf(zzz, "avg_vq : %.2f V\r\n", val->vq);
//		break;
//	default:
//		break;
//	}
